function [advanced_trans, advanced_delay] = data_trans_color_fix(N, M, R, t_last, topo)
% 1. 按链路与时间均匀生成数据包
% 2. 计算转发表，原始列表发送/接收交替
...
while sum(mark) < length(link_t)
    ...
    [table, color_table, ~] = allocate_color(link_t{index_link(sel)}(1), link_t{index_link(sel)}(2), table, color_table, topo, -1);
    [table, color_table, ~] = allocate_color(link_t{index_link(sel)}(2), link_t{index_link(sel)}(1), table, color_table, topo, -1);
end
...
while sum(mark) < length(link_t)
    ...
    [table, ~, ~] = allocate_color(link_t{index_link(sel)}(1), link_t{index_link(sel)}(2), table, color_table, topo, floor(num_allocate/2));
    [table, color_table, ~] = allocate_color(link_t{index_link(sel)}(2), link_t{index_link(sel)}(1), table, color_table, topo, floor(num_allocate/2));
end
...
while cur_time < t_last*1.5
    ...
    %         [table, color_table, ~] = allocate_color(ext_link(1), ext_link(2), table, color_table, topo, num);
    %         continue;
    %     end
    %     if flag % src主导
    %         num = min(slot_need_table(ext_slot_src, ext_slot_dst), ceil(color_table(ext_slot_src, 2) / color_table(ext_slot_src, 1)));
    %         [table, color_table, ~] = allocate_color(ext_slot_src, ext_slot_dst, table, color_table, topo, num);
    %     else    % dst主导
    %         num = min(slot_need_table(ext_slot_dst, ext_slot_src), ceil(color_table(ext_slot_dst, 2) / color_table(ext_slot_dst, 1)));
    %         [table, color_table, ~] = allocate_color(ext_slot_dst, ext_slot_src, table, color_table, topo, num);
    %     end
    % end
    % flag = 1 - flag;
    for i = 1:M
