...
while sum(mark) < length(link_t)
    ...
    [table, color_table, ~] = allocate_new(link_t{index_link(sel)}(1), link_t{index_link(sel)}(2), table, color_table, topo, -1);
    [table, color_table, ~] = allocate_new(link_t{index_link(sel)}(2), link_t{index_link(sel)}(1), table, color_table, topo, -1);
end
...
while cur_time < t_last+4
    ...
    while sum(mark) < length(link_t)
        ...
        %     [table, color_table, ~] = allocate_new(ext_link(1), ext_link(2), table, color_table, topo, num);
        %     continue;
        % end
        if flag % src主导
            num = min(slot_need_table(ext_slot_src, ext_slot_dst), ceil(color_table(ext_slot_src, 2) / color_table(ext_slot_src, 1)));
            [table, color_table, ~] = allocate_new(ext_slot_src, ext_slot_dst, table, color_table, topo, num);
        else    % dst主导
            num = min(slot_need_table(ext_slot_dst, ext_slot_src), ceil(color_table(ext_slot_dst, 2) / color_table(ext_slot_dst, 1)));
            [table, color_table, ~] = allocate_new(ext_slot_dst, ext_slot_src, table, color_table, topo, num);
        end
    end
    flag = 1 - flag;
    for i = 1:M
...
contrast_trans = throughput(2:t_last/0.01+1);
contrast_delay = delay(delay > 0);
save('contrast_trans.mat', 'contrast_trans');
save('contrast_delay.mat', 'contrast_delay');
