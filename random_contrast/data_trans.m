...
while sum(mark) < length(link_t)
    ...
    table = allocate(link_t{index_link(sel)}(1), link_t{index_link(sel)}(2), table, 1);
    table = allocate(link_t{index_link(sel)}(2), link_t{index_link(sel)}(1), table, 1);
end
...
while cur_time < t_last*2
    ...
    while sum(mark) < length(link_t)
        ...
        if flag % src主导
            ...
            table = allocate(ext_slot_src, ext_slot_dst, table, num);
        else    % dst主导
            num = slot_need_table(ext_slot_dst, ext_slot_src);
            table = allocate(ext_slot_dst, ext_slot_src, table, num);
        end
    end
